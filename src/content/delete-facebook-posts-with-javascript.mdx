...
Be sure to replace `YOUR_FACEBOOK_USERNAME{:sh}` with your actual Facebook username.
...
with a node of your choice. The node we give this function will eventually be a `span{:html}`
...
return `true{:js}` if the attribute/value pair is found, and `false{:js}` if it is not.
...
1. First, we loop through the array of button elements to open the prompt containing the "Delete"
   button for the post
2. For each button element, we trigger a `click(){:js}` method
3. Once we have the prompt containing the "Delete" button open, we use an XPath selector to scan the
   DOM for a `span{:html}` element with its text set to "Delete"
4. Then, as mentioned above, we need to find the parent element that you actually need to click to
   delete the post. Walk backwards from that `span{:html}` until you find an element with
   attribute == "menuitem"
5. Trigger another `click(){:js}` method to click the delete button element
6. Continue the loop
...
1. In our `btns{:js}` array, the `aria-label{:html}` for the "Action options" button could
   change at any time.
2. In our `checkAttrVal{:js}` function, nobody can say that the clickable parent node will be
   found with that specific attribute/value pair. They could easily change, for example,
   `role="menuitem"{:html}` to `role="button"{:html}` which would require you
   to modify this code.
3. The XPath selector we use to find the "Delete" text could change. Facebook could swap the
   `span{:html}` tag with another HTML element if they choose.
