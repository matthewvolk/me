export const metadata = {
  ...
  description: "A tutorial describing how to implement more than one local user authentication strategy using Node.js, Express.js, and Passport.js.",
  date: "2018-08-07",
  tags: ["node", "express", "passport", "authentication"],
  hidden: true,
};
...
module.exports = function(passport) {
  ...
  passport.use('local-otherUser-signup', new LocalStrategy({ // [!code ++]
    // Include logic that searches the application database // [!code ++]
    // for an existing username, if no username exists, create // [!code ++]
    // and save it to the database along with a hashed // [!code ++]
    // version of the password // [!code ++]
  }); // [!code ++]
}
```
...
app.post('/signup-2', passport.authenticate('local-otherUser-signup', { ... }); // [!code ++]
```
