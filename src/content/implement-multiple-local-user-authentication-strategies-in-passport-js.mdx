...
Although it isn't required, Passport.js's `passport.use();{:js}` method does take one optional
...
The `passport.use{:js}` method is similar to Express's `app.use{:js}` in that it mounts a
specified strategy to the Passport object, which is called when the route handler callback invokes
the `authenticate();{:js}` method (e.g., `app.get( "/path",
passport.authenticate('strategyName'));{:js}`).
...
anytime you want via the `authenticate();{:js}` method.
...
In the `app.js{:js}` file, we are importing the passport object, and then passing it as an
argument into our `config/passport.js{:js}` file which will initialize the specified strategies
in the app's Passport object.

In order to change the code above so that you can use multiple local strategies, is to add a

`passport.use{:js}` method with names of the strategies we want to include. I
...
## Extending `passport.serializeUser();{:js}` and `passport.deserializeUser();{:js}`
...
argument of the "done" function in the `serializeUser{:js}` method) is saved in the session and
...
`serializeUser{:js}` method is attached to the session as
`req.session.passport.user = {}{:js}`. The first argument of `deserializeUser{:js}`
...
If you do not make your user ID unique across each user Model, your `serializeUser{:js}`
