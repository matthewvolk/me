...
Walk through each step of the `npm init{:sh}` script (our entry point is going to be `index.js{:js}`).

When you are done, create a `.env{:sh}` file in the root of your project directory.

Open the `.env{:sh}` file in your text editor and enter:
...
Once installed, create an `index.js{:js}` file in the root directory of your project to serve as
...
Then go ahead and use `app.listen{:js}` to tell your app which port to listen for:
...
Once installed, run `nodemon{:sh}` in your terminal from within your application directory and
wait for your `Server running at: ...{:sh}` message to let you know your app is running on the
...
The module basically does us the favor of injecting different headers within our application using `res.header{:js}`,
but you can read some more on that [here](https://enable-cors.org/server_expressjs.html). We'll be using this
module with another module we installed called `body-parser{:js}`, which parses
incoming request bodies. For example, when you receive a form submission, body-parser
will help you parse the form input data. When you receive a `GET{:js}` request
...
another file without cluttering our main entry file. Go ahead and create a new constant called `users{:js}`
and have it require the file where we will store our routes. We
also need to mount another piece of middleware on the `app{:js}` variable to
add the `/users{:js}` prefix to all the routes in the file we create below.
...
If you look at nodemon, you'll notice that your app crashed, because it couldn't find the file `./routes/users{:sh}`
...
To test that everything works so far, navigate to `localhost:3000/users/authenticate{:sh}`, and
you should see `AUTHENTICATE{:sh}` on the screen.
...
- Error on initial connection. If initial connection fails, Mongoose will not attempt to reconnect,
  it will emit an `error{:js}` event, and the promise `mongoose.connect(){:js}` returns will
  reject.
- Error after initial connection was established. Mongoose will attempt to reconnect, and it will
  emit an `error{:js}` event.
...
Create a directory called `models{:sh}` and require the following modules inside a file called
...
Back in the root directory of your file navigate to `routes/users.js{:sh}` and require the
...
Navigate down to the `router.post{:js}` request for the `/register{:sh}` path and change the
...
At this point, we can go ahead and make a POST request to our application's `register{:js}`
...
requests like `curl{:sh}` or [HTTPie](https://httpie.org/).

Make your POST request to `http://localhost:3000/users/register{:sh}` and then for your post
...
\*_Note: You can refactor the response on the `/register{:sh}` `POST{:sh}` method to keep
...
In our `index.js{:sh}` file, add the following lines of code:
...
Create a folder in the root of your project directory called `config{:sh}`, then inside it
create a file called `passport.js{:sh}`.  
...
We should also import Passport.js and the JSON web token module into our `routes/users.js{:sh}`
file:
...
nodemon isn't breaking. Once you've squashed any possible bugs, we are now going to our `routes{:sh}`
directory into the `users.js{:sh}` file.
...
We also created a function we have not yet defined, called `User.comparePassword(){:js}`. Let's
...
At the bottom of the `models/user.js{:sh}` file:
...
Open Postman, open a new tab and start drafting a new `POST{:sh}` request. the address is going
to be `http://localhost:3000/users/authenticate{:sh}`. For the body of the request:
...
##### Aside: Why is the end point called `/authenticate{:sh}` and not `/login{:sh}`?

At the moment, all the `/authenticate{:sh}` endpoint does is return a JSON object with the JWT
